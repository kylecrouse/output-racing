...
module.exports = {
  ...
  getSeasonStandings: async (seasonid, carclassid, custid, page) => {
    ...
    const stats = await page.evaluate(({ seasonid, carclassid, custid }) => {
			return new Promise((resolve, reject) => {
				load(
					"/memberstats/member/GetSeasonStandings",
					{ seasonid, carclassid, clubid: -1, raceweek: -1, division: -1, start: 1, end: 1, sort: "points", order: "desc", custid },
					function (req) {
						return function() {
							if (req.readyState == 4) {
									if (req.status == 200) {
            				...
            				load(
            					"/memberstats/member/GetSeasonStandings",
            					{ seasonid, carclassid, clubid: -1, raceweek: -1, division: -1, start: data.custrow, end: data.custrow, sort: "points", order: "desc", custid },
            					function (req) {
            						return function() {
            							if (req.readyState == 4) {
            									if (req.status == 200) {
