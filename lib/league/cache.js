...
function normalizeSeason(season, drivers) {
  ...
  season.fields.standings = season.fields.standings
    ...
    .map(item => ({
      ...item, 
      driver: drivers.find(driver => driver.id === item.id)
    }));

  // Map drivers on to season results
  ...
  const [nextRace] = season.fields.schedule
  ...
  if (lastRace) {
    lastRace = {
      ...
      track: tracks.find(({ name }) => lastRace.track.match(name))
    };
  }
  ...
  return {
    id: season.sys.id,
    ...season.fields,
    results: season.fields.results.map(item => ({ 
      ...item, 
      driver: drivers.find(driver => driver.id === item.id)
    })),
    nextRace,
