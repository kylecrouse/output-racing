...
class League extends Entry {
  ...
  getLastRace(options = {}) {
    const season = Array.isArray(this.season.results) 
      && this.season.results.find(({ uploaded }) => uploaded)
        ? this.season
  ...
  getNextRace(options = {}) {
    ...
    const [race = null] = this.season.results
      .filter(item => !item.offWeek && !item.raceId)
      .filter(race => options.track && race.track
        ? race.track.indexOf(options.track) == 0
        : true
      )
      .sort((a, b) => moment(a.date).diff(b.date));
      
    if (!race) {
      const next = this.seasons.find(season => season.id != this.season.id);
      const [premiere = null] = next.schedule
        .filter(item => !item.offWeek && !item.raceId)
        .filter(race => options.track && race.track
          ? race.track.indexOf(options.track) == 0
          : true
  ...
  async mapScheduledRace(props) {
    const race = await this.getScheduledRace(props.scheduleId);
    return race.entry ? race : this.addRace(props);
  }
