  ...
  return new Promise((resolve, reject) => {
    parse(csv, {
      ...
      on_record: (record, context) => ({
        ...
        date: new Date(`${record['Race Date']} ${record['Race Time']} GMT`),
...
module.exports = {
  getSchedule: async (season_id, session) => {
    ...
    await session.goto(`http://www.simracerhub.com/scoring/season_schedule.php?season_id=${season_id}`);
    
    // Get all race result links (excluding chase rows, which are .bgBlue)
    const events = await session.$$eval(
      '#sched_table tr[id^=sch_]:not(.bgBlue) a[href^="season_race.php"]', 
      cells => cells.map(cell => cell.href.split('=').pop())
    );
    
    // Get all schedule IDs from row ID, fully inclusive to match CSV
    ...
    const csv = await getExport(
      `http://www.simracerhub.com/scoring/season_schedule_export.php?season_id=${season_id}`, 
      session
    );
