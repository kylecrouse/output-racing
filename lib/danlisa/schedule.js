...
module.exports = {
  getSchedule: async (season_id, session) => {
    ...
    const [meta, schedule] = await Promise.all([parseSeason(csv), parseSchedule(csv)]);
    
    return { ...meta, schedule };

    // const [results, standings, stats] = await Promise.all([
    //   Promise.map(
    //     schedule, 
    //     async (event) => (event.uploaded && event.raceId) 
    //       ? Object.assign(await getResults(event.raceId, session), { name: event.name }}, 
    //       : event,
    //     { concurrency: 3 }
    //   );,
    //   getStandings(season_id, session),
    //   getStats('season_id', season_id, session)
    // ]);
    // 
    // return { ...meta, results, standings, stats };
