...
module.exports = {
  waitForAuthentication: async (page, uri) => {
    ...
    const { username, password } = await getSecretValue('ORLBot/DanLisaScoring');
  	...
  	await page.goto(`https://www.simracerhub.com/scoring/login_form.php?returnto=${encodeURIComponent(uri)}`);
    ...
    return page.waitForResponse(`https://www.simracerhub.com${encodeURI(uri)}`);
