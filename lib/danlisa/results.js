...
module.exports = {
  getResults: async (race_id, page) => {
    ...
    const csv = await getExport(
      `http://www.simracerhub.com/scoring/season_race.php?race_id=${race_id}&csv=y`, 
      page
    );
  ...
  putResults: async (filePath, schedule_id, page) => {
    ...
    await waitForAuthentication(page, `/scoring/season_race_upload.php?schedule_id=${schedule_id}`)
  	...
  	await page.waitForSelector('input[name=results]', { visible: true });
    
  	const [fileChooser] = await Promise.all([
  	  page.waitForFileChooser(),
      page.$eval('input[name=results]', e => e.click()),
      ...
      console.log(`Warning while uploading ${schedule_id} to simracerhub.com`, error);
    });
