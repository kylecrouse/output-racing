...
async function importSeason(seasonId) {
  ...
  try {
    ...
    // Update the season with new data
    return existing.put({ ...season, results, standings, stats });
  }
  catch(error) {
    // Create the season with new data
    return league.addSeason({ ...season, results, standings, stats });
  }
}
