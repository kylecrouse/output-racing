...
async function importResults(raceId) {
  ...
  const results = await new Promise((resolve, reject) => {
    parse(csv, {
      ...
      if (err) reject(err);
        ...
        Promise.all(output.map(async (item) => {
  ...
  console.log(`Saving entries for race ID ${raceId}...`);
  try {
    // Get the existing race
    const event = await league.getRace(raceId);
    
    // Don't overwrite the name of existing events, to prevent changing manual edits.
    const { name, ...fields } = race.fields;
    
    // Update the race with new data
    await league.updateRace(event, { ...fields, results });
    
    console.log(`Updated results for race ID ${raceId}.`);
  } 
  catch(err) {
    ...
    if (err.name === 'NotFound') {
      ...
      await league.createRace({ raceId, ...race, results });
      
      console.log(`Added results for race ID ${raceId}.`);
    } else {
      console.log(err);
    }
  }
